package com.hqkj.newsproject.utils;
...
public class ImageLoader {
    public static void load(Context context, String url, ImageView iv) {
        Glide.with(context)
                ...
                .crossFade()
               // .error(R.mipmap.err_def_bg)
               // .placeholder(R.mipmap.default_pg)
                 .dontAnimate()
                .into(iv);
    }
    ...
    public static void load3(Context context, String url, ImageView iv) {
        Glide.with(context)
                .load(url)
                .diskCacheStrategy(DiskCacheStrategy.ALL)//让Glide既缓存全尺寸图片，下次在任何ImageView中加载图片的时候，全尺寸的图片将从缓存中取出，重新调整大小，然后缓存
                .crossFade()
              //  .placeholder(R.mipmap.big_default_bg)
                .into(iv);
    }
