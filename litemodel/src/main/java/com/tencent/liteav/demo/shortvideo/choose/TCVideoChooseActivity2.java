...
public class TCVideoChooseActivity2 extends Activity implements View.OnClickListener, ActivityCompat.OnRequestPermissionsResultCallback {
    private static final String TAG = TCVideoChooseActivity2.class.getSimpleName();
    ...
    private void loadPictureList() {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    ArrayList<TCVideoFileInfo> fileInfoArrayList = TCVideoEditerMgr.getAllPictrue(TCVideoChooseActivity2.this);
    ...
    private void loadVideoList() {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    ArrayList<TCVideoFileInfo> fileInfoArrayList = TCVideoEditerMgr.getAllVideo(TCVideoChooseActivity2.this);
    ...
    public void onClick(View v) {
        ...
        switch (id) {
            ...
            case R.id.tv_right:
                if (mType == TYPE_PUBLISH_CHOOSE) {
                    Intent intent = new Intent(TCVideoChooseActivity2.this, SuperPlayerActivity.class);
